...
namespace Munch
{
    ...
    public class AP_VR_ProfitActivity : Activity
    {
        ...
        public Button[] btns;
        public String profitURLD = "http://54.191.98.63/report.php?type=DailyProfit";
        public String profitURLW = "http://54.191.98.63/report.php?type=WeeklyProfit";
        public String profitURLM = "http://54.191.98.63/report.php?type=MonthlyProfit";
        public String profitURLY = "http://54.191.98.63/report.php?type=YearlyProfit";
        ...
        protected override async void OnCreate(Bundle savedInstanceState)
        {
            ...
            //Load up List
            JsonValue json = await JsonParsing<Task<JsonValue>>.FetchDataAsync(profitURLY);
            ...
            parsedData.Insert(0, (new AP_VR_ProfitList() { ItemName = "Name", Profit = "Earned", Date= "Time"}));
            ...
            {
                JsonValue jsondaily = await JsonParsing<Task<JsonValue>>.FetchDataAsync(profitURLD);
                List<AP_VR_ProfitList> dailyparsedData = JsonParsing<AP_VR_ProfitList>.ParseAndDisplay(jsondaily);
                mItems = dailyparsedData;
                mListView = FindViewById<ListView>(Resource.Id.ProfitReportListView);
                dailyparsedData.Insert(0, (new AP_VR_ProfitList() { ItemName = "Name", Profit = "Earned", Date = "Time" }));
            ...
            {
                JsonValue jsonweekly = await JsonParsing<Task<JsonValue>>.FetchDataAsync(profitURLW);
                List<AP_VR_ProfitList> weeklyparsedData = JsonParsing<AP_VR_ProfitList>.ParseAndDisplay(jsonweekly);
                mItems = weeklyparsedData;
                mListView = FindViewById<ListView>(Resource.Id.ProfitReportListView);
                weeklyparsedData.Insert(0, (new AP_VR_ProfitList() { ItemName = "Name", Profit = "Earned", Date = "Time" }));
            ...
            {
                JsonValue jsonmonthly = await JsonParsing<Task<JsonValue>>.FetchDataAsync(profitURLM);
                List<AP_VR_ProfitList> monthlyparsedData = JsonParsing<AP_VR_ProfitList>.ParseAndDisplay(jsonmonthly);
                mItems = monthlyparsedData;
                mListView = FindViewById<ListView>(Resource.Id.ProfitReportListView);
                monthlyparsedData.Insert(0, (new AP_VR_ProfitList() { ItemName = "Name", Profit = "Earned", Date = "Time" }));
            ...
            {
                JsonValue jsonyearly = await JsonParsing<Task<JsonValue>>.FetchDataAsync(profitURLY);
                List<AP_VR_ProfitList> yearlyparsedData = JsonParsing<AP_VR_ProfitList>.ParseAndDisplay(jsonyearly);
                mItems = yearlyparsedData;
                mListView = FindViewById<ListView>(Resource.Id.ProfitReportListView);
                yearlyparsedData.Insert(0, (new AP_VR_ProfitList() { ItemName = "Name", Profit = "Earned", Date = "Time" }));
